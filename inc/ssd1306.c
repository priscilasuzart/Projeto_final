...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
    ...
    if (c >= 'A' && c <= 'Z') {
      ...
      index = (c - '-' + 74) * 8;   // Para dois pontos
    }
    else if (c == ':') {
      index = (c - ':' + 77) * 8;   // Para dois pontos
    }
    else if (c == '?') {
      index = (c - '?' + 79) * 8;   // Para interrogação
    }
    else if (c == '!') {
      index = (c - '!' + 80) * 8;   // Para exclamação
    }

    // Adicionar mais caracteres especiais conforme necessário
